...
class EmbeddingBuilder(Callback):
            ...
            for set_idx, (datatype, dataset) in enumerate(datasets.items()):
                for batch_idx, batch in enumerate(dataset):
                    ...
                    if (
                        not os.path.exists(
                            os.path.join(
                                self.output_dir, datatype, os.path.basename(str(batch.event_file))
                            )
                        )
                    ) or self.overwrite:
    ...
    def save_downstream(self, batch, pl_module, datatype):
        ...
        with open(
            os.path.join(self.output_dir, datatype, os.path.basename(str(batch.event_file))), "wb"
        ) as pickle_file:
            torch.save(batch, pickle_file)

        logging.info("Saved event {}".format(os.path.basename(str(batch.event_file))))
